...
const addPurchases = (arr) => arr.reduce((acc, num) => acc + num.purchasePrice, 0);
...
// How do?
// const returnNames = (arr) => arr.reduce((acc, names) => acc.push(names.name), []);

const returnNames = (arr) => {
  let namesArray = arr.reduce((acc, names) => {
    acc.push(names.name);
    return acc;
  }, []);
  return namesArray;
...
const reversedString = (str) => str.split('').reduce((acc, string) => string + acc, '');
