...
const screenForNames = (arr) => arr.map((surname) => surname.match(/^\w[D|M]r\./));
...
const toTitleCase = (arr) => arr.map((upper) => upper[0].toUpperCase() + upper.slice(1));
...
let biggerThanLuke = (arr) => arr.filter((heaviestSoFar, nextCharacter) => Number(heaviestSoFar.mass) < Number(nextCharacter.mass) ? heaviestSoFar : nextCharacter).name;
