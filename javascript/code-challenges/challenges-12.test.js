...
const validatePin = (pin) => (/^\d{4}$|^&/gm).test(pin);
...
const validateEmail = (email) => /^[0-9=?A-Z^_a-z`{|}~](\.?[0-9=?A-Z^_a-z`{|}~])*@[a-zA-Z0-9](-*\.?[a-zA-Z0-9])*\.[a-zA-Z](-?[a-zA-Z0-9])+$/.test(email);
// Code found at https://masteringjs.io/tutorials/fundamentals/email-validation
/* ------------------------------------------------------------------------------------------------
CHALLENGE 6
...
const validatePhoneNumber = (phoneNumber) => /^(\+0?1\s)?\(?\d{3}\)?[\s.-]\d{3}[\s.-]\d{4}$/.test(phoneNumber);
